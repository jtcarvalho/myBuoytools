...
for station_id in stations:
    ...
    if common_dates is not None:
        ...
        # Convert common_dates to numpy array
        common_dates_np = common_dates.to_numpy()

        # Ensure common_dates are datetime64[ns] type
        if not np.issubdtype(common_dates_np.dtype, np.datetime64):
            common_dates_np = common_dates_np.astype('datetime64[ns]')

        # Select common dates
        buoy_data_common = buoy_data.sel(time=common_dates_np)
        model_data_common = model_data.sel(time=common_dates_np)
        ...
        # Calcula NBIAS e NRMSE apenas se houver dados em comum
